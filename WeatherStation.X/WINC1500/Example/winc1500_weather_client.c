...
#define PREFIX_BUFFER            "GET /v1/forecast?"
#define POST_BUFFER              "&current_weather=true&timezone=auto  HTTP/1.1\r\nHost: api.open-meteo.com\r\nAccept: /\r\n\r\n"

#define WEATHER_SERVER_NAME    "api.open-meteo.com"  // Weather information provider server
#define COORDS_MESS_SIZE       31                    // Message size containing coordinates + NULL
...
void ApplicationTask(void)
{
    ...
    case APP_STATE_START:
        printf("\r\n=========\r\n");            
        printf("Weather Client\r\n");
        printf("ssid: %s\r\n", WLAN_SSID);
        printf("host: %s\r\n", WEATHER_SERVER_NAME);
        printf("port: %u\r\n", SERVER_PORT);
        printf("=========\r\n");
        
        printf("-- WINC1500 weather client --\r\n");
        printf("Starting ...\r\n");
...
static void socket_cb(SOCKET sock, uint8_t message, void *pvMsg)
{
    ...
    if (sock == tcp_client_socket) 
    {
        ...
        case M2M_SOCKET_RECV_EVENT:
        {
            ...
            {
                decodeMessage((char *)pstrRecv->p_rxBuf, currentWeather );  
                close(tcp_client_socket);
                tcp_client_socket = -1;
            }
            else 
